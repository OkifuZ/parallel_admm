...
namespace __GEIGEN__ {
	...
	__device__ __host__ Matrix9x9d __v9_vec9_toMat9x9(const Vector9& a, const Vector9& b, const float& coe = 1);
	...
	__device__ __host__ void __NewtonSolverForCubicEquation(const float& a, const float& b, const float& c, const float& d, float* results, int& num_solutions, float EPS = 1e-6);

	__device__ __host__ void __NewtonSolverForCubicEquation_satbleNeohook(const float& a, const float& b, const float& c, const float& d, float* results, int& num_solutions, float EPS = 1e-6);

	__device__ __host__ void __SolverForCubicEquation(const float& a, const float& b, const float& c, const float& d, float* results, int& num_solutions, float EPS = 1e-6);
	...
	__device__ __host__ void __makePD2x2(const float& a00, const float& a01, const float& a10, const float& a11, float eigenValues[2], int& num, float2 eigenVectors[2], float eps = 1e-32);
