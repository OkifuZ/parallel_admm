    ...
    *stack_ptr++ = 0;
    ...
    do
    {
        ...
        if (overlap(_bv, _bvs[L_idx], gapl))
        {
            ...
            {
                if (idx != _faces[obj_idx].x && idx != _faces[obj_idx].y && idx != _faces[obj_idx].z) {
                    _checkDCDPointTriangle(_vertexes, idx, _faces[obj_idx].x, _faces[obj_idx].y, _faces[obj_idx].z, dHat, _cpNum, _collisionPair, contact_info);
                }
            }
            else // the node is not a leaf.
            {
...
void lbvh_f::discreteCollisionDetection(double thickness) {
    ...
    _dcd_vf<<< blockNum, threadNum >>>(this->_vertexes, this->_faces, this->_surfVerts, this->_bvs, this->_nodes, 
        this->_collisionPair, this->_cpNum, this->_dcd_info, thickness, this->vert_number);
    CUDA_SAFE_CALL(cudaDeviceSynchronize());
}
