...
public:
	...
	ADMMSolverFull() : Solver() {
		
	}

	// TODO: handle W better 
	...
	//std::unique_ptr < Eigen::PardisoLDLT<Eigen::SparseMatrix<ADU::Real>>> m_Pardiso_LLT_solver;

};



class ADMMSolverFull_RL_damping :public ADMMSolverFull {
public:
	...
	virtual void project_feasible(ADU::Matf_X3& p,
	...
	ADU::Matf_X3 g_km1_e;
	ADU::Real g_k_sqn_e;
	ADU::Real g_km1_sqn_e;
	ADU::Matf_X3 p_cg_e;


	std::vector<ProximalQuery::ContactInfoList> contact_islands; // TODO
	...
	std::vector<std::vector<ADU::Real>> primal_residual;
	std::vector<std::vector<ADU::Real>> dual_residual;
	std::vector<std::vector<ADU::Real>> combined_residual;
