...
public:
    ...
    static void geometry_to_constraints(
        MeshData& mesh_data, size_t mesh_id, const PhyxMaterial& material, std::vector<std::shared_ptr<Constraint>>& constraints, std::vector<std::shared_ptr<XPBDConstraint>>& XPBD_constraints)
    {
        using namespace ADU;
